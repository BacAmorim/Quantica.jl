...
#######################################################################
"""
    bandstructure(h::Hamiltonian, mesh::Mesh; cut = missing, minoverlap = 0.5, method = defaultmethod(h), transform = missing)
...
# See also
    `marchingmesh`
"""
bandstructure(; kw...) = h -> bandstructure(h; kw...)
bandstructure(mesh::Mesh; kw...) = h -> bandstructure(h, mesh; kw...)
...
function bandstructure(h::Union{Hamiltonian,ParametricHamiltonian}, mesh::Mesh;
                       method = defaultmethod(h), cut = missing, minoverlap = 0.5,
                       transform = missing)
...
function bandstructure(matrixf::Function, mesh::Mesh;
                       ...
                       minoverlap = 0.5, transform = missing)
